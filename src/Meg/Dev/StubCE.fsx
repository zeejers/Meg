...
type StubBuilder() =
    ...
    member inline this.Bind2
        (input: Internal<'T1>, input2: Internal<'T2>, [<InlineIfLambda>] f: ('T1 * 'T2 -> Internal<'U>))
        : Internal<'U> =
        raise <| NotImplementedException()
    ...
    member inline this.Bind2Return
        (input: Internal<'T1>, input2: Internal<'T2>, [<InlineIfLambda>] f: ('T1 * 'T2 -> 'U))
        : Internal<'U> =
        raise <| NotImplementedException()
    ...
    member inline this.MergeSources3
        (input: Internal<'T1>, input2: Internal<'T2>, input3: Internal<'T3>)
        : Internal<'T1 * 'T2 * 'T3> =
        raise <| NotImplementedException()
    ...
    member inline this.While
        ([<InlineIfLambda>] guard: unit -> bool, [<InlineIfLambda>] body: Delayed<'T>)
        : Internal<'T> =
        raise <| NotImplementedException()

    /// Called for while...do expressions in computation expressions.
    member inline this.While
        ([<InlineIfLambda>] guard: unit -> bool, [<InlineIfLambda>] body: Delayed<unit>)
        : Internal<unit> =
        raise <| NotImplementedException()
    ...
    member inline this.TryFinally
        ([<InlineIfLambda>] body: Delayed<'T>, [<InlineIfLambda>] final: unit -> unit)
        : Internal<'T> =
        raise <| NotImplementedException()

    /// Called for try...finally expressions in computation expressions.
    member inline this.TryWith
        ([<InlineIfLambda>] body: Delayed<'T>, [<InlineIfLambda>] failure: exn -> Internal<'T>)
        : Internal<'T> =
        raise <| NotImplementedException()

    /// Called for use bindings in computation expressions.
    member inline this.Using
        (resource: 'T :> IAsyncDisposable, [<InlineIfLambda>] f: 'T -> Internal<'U>)
        : Internal<'U> =
